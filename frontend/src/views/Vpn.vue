    ...
    <div class="vpn-grid">
      ...
      <section class="vpn-card egress" :class="egressState">
        ...
        <div v-else class="empty-vpn">
          No egress VPN provider is configured yet. This project will add ProtonVPN/generic VPN support and per-app routing controls.
        </div>
...
<style scoped>
.vpn-page { max-width: 1180px; margin: 0 auto; }
.vpn-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(360px, 1fr)); gap: var(--space-4); }
.vpn-card {
  background: var(--bg-1); border: 1.5px solid var(--border); border-radius: var(--radius);
  padding: var(--space-4); box-shadow: var(--shadow-1); display: flex; flex-direction: column; gap: var(--space-3);
}
...
.vpn-card-head { display: flex; align-items: flex-start; justify-content: space-between; gap: var(--space-3); }
.vpn-card h2 { margin: 0 0 3px; font-size: 18px; }
.vpn-card p { margin: 0; color: var(--fg-2); font-size: 12.5px; }
.status-pill { font-size: 11px; font-weight: 800; text-transform: uppercase; padding: 4px 10px; border-radius: 999px; background: var(--bg-2); }
...
.signal { border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 8px; background: var(--bg-0); }
.signal.ok { border-color: rgba(22,163,74,0.35); }
.signal.warn { border-color: rgba(217,119,6,0.35); }
.signal span, .info-grid span { display: block; font-size: 10px; color: var(--fg-2); text-transform: uppercase; letter-spacing: 0.06em; }
.signal strong, .info-grid strong { display: block; margin-top: 3px; font-size: 12px; color: var(--fg-0); word-break: break-word; }
.info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
.info-grid > div { background: var(--bg-0); border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 8px; }
.info-grid .wide { grid-column: 1 / -1; }
.issues { color: var(--warn); font-size: 12px; display: grid; gap: 4px; }
.provider-list { display: grid; gap: 8px; }
.provider-row { display: grid; grid-template-columns: auto 1fr auto; gap: 10px; align-items: center; background: var(--bg-0); border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 8px; }
...
.empty-vpn { color: var(--fg-2); font-size: 13px; background: var(--bg-0); border: 1px dashed var(--border-strong); border-radius: var(--radius-sm); padding: var(--space-4); }
.future-flow { display: flex; align-items: center; justify-content: center; gap: 8px; color: var(--fg-1); font-size: 12px; }
