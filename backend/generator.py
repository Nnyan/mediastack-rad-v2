...
def _render_traefik(request: StackRequest, domain: str | None) -> dict[str, Any]:
    ...
    labels = [
        "traefik.enable=true",
        f"traefik.http.routers.traefik-dashboard.rule=Host(`traefik.{domain}`)",
        f"traefik.http.routers.traefik-dashboard.entrypoints=websecure",
        f"traefik.http.routers.traefik-dashboard.tls=true",
        f"traefik.http.routers.traefik-dashboard.tls.certresolver={request.cert_resolver}",
        f"traefik.http.routers.traefik-dashboard.service=api@internal",
    ]

    if request.tinyauth_enabled and _has_tinyauth(request):
        lan = (request.lan_subnet or "10.0.0.0/22").strip()
        labels += [
            f"traefik.http.routers.traefik-dashboard-lan.rule=Host(`traefik.{domain}`) && ClientIP(`{lan}`)",
            f"traefik.http.routers.traefik-dashboard-lan.entrypoints=websecure",
            f"traefik.http.routers.traefik-dashboard-lan.priority=10",
            f"traefik.http.routers.traefik-dashboard-lan.tls=true",
            f"traefik.http.routers.traefik-dashboard-lan.tls.certresolver={request.cert_resolver}",
            f"traefik.http.routers.traefik-dashboard-lan.service=api@internal",
            f"traefik.http.routers.traefik-dashboard.priority=5",
            f"traefik.http.routers.traefik-dashboard.middlewares=tinyauth-auth@docker",
        ]
...
api:
  dashboard: true
  # insecure: true exposes the API on :8081 inside the container only.
  # Port 8081 is NOT mapped to the host — the dashboard is routed
  # through Traefik itself at traefik.<domain> with Tinyauth auth.
  # RAD's backend proxy still reaches the API via the Docker network.
  insecure: true
